...
%parameters
enable_plots=false; %do you wish to plot the WLC? 
...
fprintf(['\n>>>[task3 Deprecated - N for] Starting Computation WLC 3D with %u'...
...
if enable_error_plot
    ...
    hold off
    title(sprintf('[task3 Deprecated - N for]Length of chain versus the squared end-to-end distance [3D]; N=%u',N))
...
fprintf('\n>>>[task3 Deprecated - N for] Completed.\n')
...
%%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%%^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
%% Start - Fully Vectorised

% PARAMETERS SET AT THE TOP OF THE PAGE
...
%opening statement (for console iterpretability)
fprintf(['\n>>>[task3 Deprecated - Full Vec] Starting Computation WLC 3D with %u'...
        ' configurations each with %u iterations and number of segments '...
        'between %u and %u'],P,N,min(K),max(K))
...
if enable_error_plot
    ...
    plot(length_chain,theoretical_approx)
    hold off
    title(sprintf('[task3 Deprecated - Full Vec]Length of chain versus the squared end-to-end distance [3D]; N=%u',N))
    xlabel('Length of chain [nm]')
    ylabel('squared end to end distance [nm^2]')
    legend('Monte Carlo','Theoretical Values','Theoretical Values (using approximation)')
end
...
%closing statement (for console iterpretability)
fprintf('\n>>>[task3 Deprecated - Full Vec] Completed.\n')